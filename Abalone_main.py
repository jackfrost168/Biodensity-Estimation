...
def step_decay(ep):
    ...
    print('Learning rate for epoch {} is {}.'.format(ep + 1, lrate))
...
optimizer = Adam(lr=1e-5, decay=5e-4)
...
with open('data/Abalone/models/{}.json'.format(net), 'w') as fout:
...
for ep in range(epoch):
    ...
    for idx_train in range(0, len_train, batch_size):
        ...
        model.fit(x, y, batch_size=2 ,verbose=0)
...
shutil.move('data/Abalone/weights_{}'.format(dataset), weights_dir_neo)
...
model = model_from_json(open('data/Abalone/models/{}.json'.format(net), 'r').read())
