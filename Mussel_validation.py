...
net = 'countingnet'
dataset = 'A'
#weights_dir = 'data/Mussel/weights_' + dataset
...
model = model_from_json(open('data/mussel300/models/{}.json'.format(net), 'r').read())
#model.load_weights(os.path.join(weights_dir_neo, '{}_best.hdf5'.format(net)))
#model.load_weights("/mnt/jackfrost/counting/data/mussel300/weights_A/countingnet_MAE13.736_RMSE19.982_SFN0.0_MAPE0.531_epoch17-21.0.hdf5")
model.load_weights("/mnt/jackfrost/counting/data/mussel300/wnet_MAE7.859_RMSE10.835_SFN0.0_MAPE0.385_epoch96-21.0.hdf5")
ct_preds = []
ct_gts = []
...
for worst_idx in idx_max_error[:28].tolist() + [dis_idx]:
    ...
    ax_y.set_title('Ground_truth: ' + str(round(np.sum(test_y_display), 2)))#reserve 2 after dot
    ax_pred.imshow(pred, cmap=plt.cm.jet)
    ax_pred.set_title('Prediction: ' + str(round(np.sum(pred), 2)))
    plt.show()
...
# test_img_paths = []
# root = 'data/mussel300/test_data/images/'    #change
# paths = os.listdir(root)
# for i in range(len(paths)):
#     tmp_img_paths = os.path.join(root, paths[i])
#     test_img_paths.append(tmp_img_paths)

# print("test_img_paths:", len(test_img_paths))
# print("test_img_paths:", test_img_paths[0])
# #print("test_dm_paths:", len(test_dm_paths))
# # Generate raw images(normalized by imagenet rgb) and density maps
# #test_x, test_y = gen_var_from_paths(test_img_paths[:]), gen_var_from_paths(test_dm_paths[:], stride=2)
# test_x = gen_var_from_paths(test_img_paths[:])
# print("test_x.shape", test_x.shape)
...
# weights_dir_neo = 'weights_A_MSE_BCE_bestMAE0.344_Wed-Oct-23'
# model = model_from_json(open('data/mussel300/models/{}.json'.format(net), 'r').read())
# model.load_weights("/mnt/jackfrost/counting/data/mussel300/countingnet_MAE7.859_RMSE10.835_SFN0.0_MAPE0.385_epoch96-21.0.hdf5")
# # ct_preds = []
# # ct_gts = []
# # for i in range(len(test_x[:])):
# #     if i % 100 == 0:
...
# # Show the 5 worst samples
# #for worst_idx in idx_max_error[:28].tolist() + [dis_idx]:
# for worst_idx in range(len(test_img_paths)):
#     test_x_display = np.squeeze(test_x[worst_idx])
#     #test_y_display = np.squeeze(test_y[worst_idx])
#     path_test_display = test_img_paths[worst_idx]
#     pred = np.squeeze(model.predict(np.expand_dims(test_x_display, axis=0)))
