...
def gen_paths_img_dm(path_file_root='data/paths_train_val_test', dataset='A'):
    ...
    #print(len(path_file_root_curr))
    img_paths = []
    dm_paths = []
    paths = os.listdir(path_file_root_curr)[:2] #['paths_test.txt', 'paths_train.txt']
    print(paths)
...
def gen_var_from_paths(paths, stride=1, unit_len=16):
    ...
    if format_suffix == 'h5':
        for ph in paths:
            ...
            #i=i+1
            #if i>5:
             #   print(np.array(vars[i-1]).shape)
    elif format_suffix == 'JPG':
        for ph in paths:
            #####notice!!!!!!!!!!!!!!!!!!!!!!
            ###shrink
            raw = cv2.cvtColor(cv2.imread(ph), cv2.COLOR_BGR2RGB)
            #raw = cv2.resize(raw, (int(raw.shape[1] / 8), int(raw.shape[0] / 8)),
...
def gen_density_map_gaussian(im, points, sigma=4):
    ...
    for p in points:
        ...
        #print(gaussian_map.shape[0])
        if gaussian_map.shape[0] == 0 or gaussian_map.shape[1] == 0:
            print("question:", gaussian_map.shape)
            continue
        density_map[
